...
def my_model(spark, monitoring_dir, checkpoint_dir, time_step_interval, current_time, seconds_horizon):
    ...
    inputSDF1 = inputSDF.filter((col("date").between(starttime, endtime)) & (col("atStop") == "1")).select(col("vehicleID"), col('date').alias("time") ,col('closerStopID').alias("stop"),col('my_time'))

    inputSDF1 = inputSDF1.withColumn("newtime",concat("time",lit("_"),"stop")).select(col("newtime"),col("vehicleID"),col("my_time"))

    watermark_inputSDF = inputSDF1.withWatermark("my_time", "0 seconds")


    solutionSDF = watermark_inputSDF.groupBy(pyspark.sql.functions.window("my_time", my_frequency, my_frequency),
                                             pyspark.sql.functions.col("vehicleID")).agg(sort_array(collect_list("newtime")).alias("newtime"))

    #split_col = pyspark.sql.functions.split(solutionSDF['newtime'], ',')

    solutionSDF = solutionSDF.withColumn("newtimeandstop", explode("newtime"))

    split_col = pyspark.sql.functions.split(solutionSDF['newtimeandstop'], '_')

    solutionSDF = solutionSDF.withColumn("time",split_col.getItem(0))
    solutionSDF = solutionSDF.withColumn("stop", split_col.getItem(1))
    solutionSDF = solutionSDF.drop("window","newtime","newtimeandstop")
