...
def process_line(line):
    ...
    if (len(params_list) == 10):
        process_weekday = datetime.strptime(params_list[0], '%Y-%m-%d %H:%M:%S').weekday()
        if (process_weekday not in [5, 6]):
    ...
    return res
#average function
def function_avg(nvalues, values):
  return round((sum(nvalues[0]) / len(nvalues[0]) ),2)

# --------------------------
# FUNCTION my_state_update
# ------------------------------------------
def my_state_update(nvalues, values):
    res = 0
    if values is None:
        values = []
...
def my_model(ssc, monitoring_dir, bus_stop, bus_line, hours_list):
    ...
    #     print(inputDStream.pprint())
    #processed_Rdd = inputDStream.map(process_line)
    inputDStream = inputDStream.map(lambda p: p.split(","))
    sliding_duration = 1
    window_size = 5

    #use window function because i tried with updatestatekey bit was giving error (serialize error)
    inputDStream = inputDStream.window(window_size * time_step_interval, sliding_duration * time_step_interval)

    # filter data by Bus_stop,Busline and hourslist and weekdays
    solutionDStream = inputDStream.filter(lambda x: x and x[1] == str(bus_line) and x[9] == "1" and x[8] == str(bus_stop)
                                                       and x[0].split()[1].split(":")[0] in hours_list
                                                       and calendar.day_name[datetime.strptime(x[0],'%Y-%m-%d %H:%M:%S').weekday()] not in("Sunday","Saturday")  )



    # select only hours and delay
    solutionDStream1 = solutionDStream.map(lambda x: (x[0].split()[1].split(":")[0], int(x[6])))

    #groupby values
    flatDStream = solutionDStream1.groupByKey().mapValues(lambda x: round((sum(x) / len(x)), 2))

    #sort the data
    solutionDStream = flatDStream.transform(lambda rdd: rdd.sortBy(lambda x: x[1]))
