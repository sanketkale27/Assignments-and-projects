...
def fun1(rdd):
    if len(rdd.collect()) > 0:
        maxval = rdd.max(lambda p: p[2])
        return rdd.filter(lambda p: p[2] == maxval[2]).map(lambda p : (p[0],p[1])).sortBy(lambda p : p[1])
    else:
        return rdd
...
def my_model(ssc, monitoring_dir, vehicle_id):
    ...
    # filter the by the vehicale ID
    solutionDStream = inputDStream.filter(lambda p: p[7] == str(vehicle_id))

    # select the day and buslines
    solutionDStream1 = solutionDStream.map(lambda p: ((p[0].split(" ")[0]).split("-")[2], int(p[1])))

    # groupby the data
    solutionRDD3 = solutionDStream1.groupByKey().mapValues(lambda p : sorted((list(set(p))),reverse=False))

    # split the columns
    solutionRDD4 = solutionRDD3.map(lambda line: (line[0], line[1], len(line[1])))


    # using transform call funtion and calculate the max lenght and return the data
    solutionDStream5 = solutionRDD4.transform(fun1)


    # Operation A1: 'pprint' to get all results
    solutionDStream5.pprint()
