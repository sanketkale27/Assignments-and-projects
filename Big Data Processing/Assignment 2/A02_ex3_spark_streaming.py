...
# FUNCTION my_model
# ------------------------------------------
def my_model(ssc, monitoring_dir, time_step_interval, threshold_percentage):
    # 1. Operation C1: 'textFileStream' to load the dataset into a DStream
    inputDStream = ssc.textFileStream(monitoring_dir)

    inputDStream1 = inputDStream.map(lambda p: p.split(","))

    sliding_duration = 1
    window_size = 2
    # processed_Rdd.pprint()
    solutionDStream1 = inputDStream1.window(window_size * time_step_interval, sliding_duration * time_step_interval)

    solutionDStream2 = solutionDStream1.map(lambda p: ((p[0].split()[0].split("-")[2], p[0].split()[1].split(":")[0]), (int(p[3]))))
    #solutionRDD1.pprint()
    #solutionRDD2 = solutionRDD1.groupByKey.mapValues(lambda p : p)

    solutionDStream3 = solutionDStream2.groupByKey().mapValues(lambda x: round((sum(x) / len(x))*100, 2))

    # select the data which is greater than threshold_percentage
    solutionDStream4 = solutionDStream3.filter(lambda p: (p[1] > threshold_percentage))

    solutionDStream5 = solutionDStream4.map(lambda p : (p[0][0],p[0][1],p[1]))


    solutionDStream6 = solutionDStream5.transform(lambda rdd: rdd.sortBy(lambda x: x[2],ascending=False))

    #solutionRDD7= solutionRDD6.map(lambda p: (p[0], p[1], p[2]))

    #solutionRDD8.pprint()
    # TO BE COMPLETED

    # Operation A1: 'pprint' to get all results
    solutionDStream6.pprint()
...
# FUNCTION create_ssc
# ------------------------------------------
def create_ssc(sc, time_step_interval, monitoring_dir, threshold_percentage):
    ...
    my_model(ssc, monitoring_dir, time_step_interval, threshold_percentage)
...
if __name__ == '__main__':
    ...
    source_dir = "A02_ex3_micro_dataset/"
