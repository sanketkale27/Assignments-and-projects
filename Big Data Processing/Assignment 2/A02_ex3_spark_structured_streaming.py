...
# FUNCTION my_model
# ------------------------------------------
def my_model(spark, monitoring_dir, checkpoint_dir, time_step_interval, threshold_percentage):
    ...
    # create the day ,hour, my_time column
    inputSDF = inputSDF.withColumn('day', split_col.getItem(0).substr(9, 10))
    ...
    #groupby Data
    watermark_inputSDF = inputSDF.withWatermark("my_time", "0 seconds")
    solutionSDF = watermark_inputSDF.groupBy(pyspark.sql.functions.window("my_time", my_window_duration_frequency, my_sliding_duration_frequency),
                                             pyspark.sql.functions.col("day"),pyspark.sql.functions.col("hour")).mean("congestion")

    # group by data and create batch files
    solutionSDF1 = solutionSDF.withColumn("window_start", pyspark.sql.functions.col("window").start.cast("string")) \
                .withColumn("window_end", pyspark.sql.functions.col("window").end.cast("string"))

    # create precentage
    solutionSDF2 = solutionSDF1.withColumn('percentage', (round((col("avg(congestion)") * 100),2))).filter(col("percentage") >  threshold_percentage)

    # delete columns
    solutionSDF3 = solutionSDF2.drop("avg(congestion)","window","window_start","window_end")


    # Operation O1: We create the DataStreamWritter, to print by console the results in complete mode
    myDSW = solutionSDF3.writeStream\
                       .format("console") \
...
def my_main(spark,
    ...
    dsw = my_model(spark, monitoring_dir, checkpoint_dir, time_step_interval, threshold_percentage)
