...
def my_main(spark, my_dataset_dir, bus_stop, bus_line, hours_list):
    ...
    inputDF = inputDF.withColumn('datenew', split_col.getItem(0))

    #create a column hour
    inputDF = inputDF.withColumn('hour', split_col.getItem(1).substr(0, 2))

    #select the weekdays
    solutionDF1 = inputDF.filter("dayofweek(date) not in (1,7)")

    #select the data according to busline ,closerstop and hours list
    solutionDF = solutionDF1.filter((col("busLineID") == bus_line) & (col("closerStopID") == bus_stop) & (col("atStop") == "1") & (col("hour").isin(hours_list)))

    #groupby the data by hours and sort by average of delay
    finalresult = solutionDF.groupBy('hour').avg("delay").select("hour",f.round(col("avg(delay)"),2).alias("averageDelay")).sort("averageDelay")

    #collect the results in list
    resVAL = finalresult.collect()
