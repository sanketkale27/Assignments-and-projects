...
def getdaysbusline(daysbulinelist):
    days, busline = daysbulinelist
    return (days, sorted(list(set(busline)),reverse=False))
# ------------------------------------------
# FUNCTION my_main
# ------------------------------------------
def my_main(sc, my_dataset_dir, vehicle_id):
    ...
    solutionRDD1 = inputRDD1.filter(lambda p : p[7] == str(vehicle_id) )

    #select the date and buslineID
    solutionRDD2 = solutionRDD1.map(lambda p: ((p[0].split(" ")[0]).split("-")[2], [int(p[1])]))

    #combine the data by key value pair
    solutionRDD3 = solutionRDD2.reduceByKey(lambda p, q: p + q)

    #take the busline id and make a list
    solutionRDD4 = solutionRDD3.map(getdaysbusline).sortByKey()

    #collect the data in list
    m = solutionRDD4.collect()

    #select max count of buslineid
    max_len = max([len(i[1]) for i in m])

    #filter the data of max busline id
    result = solutionRDD4.filter(lambda p: len(p[1]) == max_len)

    #filter the data by date and buslineid
    result = result.filter(lambda p : (p[0],(p[1])))

    # Operation A1: 'collect' to get all results
    resVAL = result.collect()
    for item in resVAL:
        print(item)
