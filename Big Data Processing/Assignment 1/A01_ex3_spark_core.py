...
def process_line(line):
    ...
    # 4. We return res
    return res
def getdaytimecong(daytimelist):
    day, time,cong = daytimelist
    return ((day,time) ,cong)

def getdaytimecong1(daytimelist):
    day,cong = daytimelist
    day,time = day
    return (day,time ,cong)
# ------------------------------------------
# FUNCTION my_main
...
def my_main(sc, my_dataset_dir, threshold_percentage):
    ...
    inputRDD = inputRDD.map(lambda p: p.split(","))



    # select the date,time and congestion
    solutionRDD1 = inputRDD.map(lambda p: (p[0].split(" ")[0].split("-")[2] ,  p[0].split(" ")[1].split(":")[0], [(int(p[3]))]))

    #combine the list of day time
    solutionRDD2 = solutionRDD1.map(getdaytimecong).sortByKey()

    #select key value pair of date,time and congestion
    solutionRDD3 = solutionRDD2.reduceByKey(lambda p, q: p + q)

    #calculate the percentage of congestion
    solutionRDD4 = solutionRDD3.map(lambda p: (p[0], round(sum(p[1]) / len(p[1]) * 100, 2))).sortByKey()

    #select the data which is greater than threshold_percentage
    solutionRDD5 = solutionRDD4.filter(lambda p: (p[1] > threshold_percentage))

    #sort the data by threshold_percentage
    solutionRDD6 = solutionRDD5.map(getdaytimecong1).sortBy(lambda p : p[2],ascending=False)

    #collect the data in list
    resVAL = solutionRDD6.collect()
